...
public class Test {
	public static void determineTestSequence() {
		...
		Initialization.controllerEncoderPairs
				.add(new SmartController("FL", Initialization.FL, Initialization.leftDriveEncoder, false));
		Initialization.controllerEncoderPairs
				.add(new SmartController("FR", Initialization.FR, Initialization.rightDriveEncoder, true));
		Initialization.controllerEncoderPairs
				.add(new SmartController("BL", Initialization.BL, Initialization.leftDriveEncoder, false));
		Initialization.controllerEncoderPairs
				.add(new SmartController("BR", Initialization.BR, Initialization.rightDriveEncoder, true));
		Initialization.testSequence = new ArrayList<>();
		if (SmartDashboard.getBoolean("Motor Series Test", false)) {
			Initialization.testSequence.add((Void) -> motorSeriesTest(Initialization.controllerEncoderPairs,
					Initialization.prefs.getDouble("Test Power", 0.6), 5));
		} else if (SmartDashboard.getBoolean("Motor Individual Test", false)) {
			Initialization.testSequence
					.add((Void) -> motorIndividualTest(Initialization.controllerEncoderPairs.get(3), 5));
		} else if (SmartDashboard.getBoolean("Elevator Speed Range Test", false)) {
			SmartController elevator = new SmartController("Elevator", Initialization.elevator,
					Initialization.elevatorEncoder, false);
